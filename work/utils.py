                                ...
                                if not ecom.event_type == ev.event_type and ecom.event_type == mir.event_type and mcom.event_type == ev.event_type:
                                    ...
                                    if not ev.commitment.event_type == ev.event_type:
                                        ...
                                        if mir.commitment == ev.commitment:
                                            ...
                                            other = comss.exclude(event_type=ev.commitment.event_type)
                                            if len(other) == 1:
                                                other = other[0]
                                                if other.event_type == ev.event_type and not other.event_type == mir.event_type:
                                                    logger.warning("REPAIR ev.event_type !! ")
                                                    ev.commitment = other
                                                    #ev.save()
